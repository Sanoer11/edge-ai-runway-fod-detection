"""
edge_logic.py - Mac webcam version
Camera -> YOLOv10s -> MQTT -> Dashboard
"""
...
def main():
    ...
    print("[Sentinel] Camera ready. Running pipeline...")
    ...
    try:
        while True:
            ...
            if detections:
                names = [(d["class"], f"{d['confidence']*100:.0f}%") for d in detections]
                print(f"[{cycle:04d}] DETECTED: {names}")
            else:
